...
//             School.find(): Retrieves all documents from the School collection.

// .select([...]): Specifies which fields to exclude from the result.

// A minus (-) before a field name means "exclude this field."
        ...
        form.parse(req, (err, fields, files) => {
            ...
            School.find({ email: fields.email }).then(resp => {
                if (resp.length > 0) {
                    ...
                    let originalFileName = photo.originalFilename.replace(" ", "_")//////it replaces the first space in file name with underscore

                    let newPath = path.join(__dirname, '../../frontend/public/images/uploaded/school', '/', originalFileName)

                    let photoData = fs.readFileSync(oldPath);////Synchronously reads the contents of a file located at oldPath
                    ///fs.writeFile is used to create or overwrite a file with specified data asynchronously.
